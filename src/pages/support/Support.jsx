...
const faqItems = [
  { id: 1, question: "유모차 반입이 가능한가요?", answer: "네, 가능합니다." },
  {
    id: 2,
    question: "외부 음식물 반입이 가능한가요?",
    answer: "물과 이유식, 특수식을 제외한 외부 음식물은 반입이 불가합니다.",
  },
  {
    id: 3,
    question: "재입장이 가능한가요?",
  ...
  },
  {
    id: 5,
    question: "수유실이 있나요?",
    answer: "행사장 내부에는 없으며 본 건물 1층 수유실을 이용해주세요.",
  },
  {
    id: 6,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              {contactItems.map((contact, index) => (
                <div
                  key={contact.id}
                  id={contact.id}
                  className={`${index === 0 ? "pb-[30px] md:border-r md:border-[#292929]/10 md:pb-0 md:pr-[45px]" : "border-t border-[#292929]/10 pt-[30px] md:border-t-0 md:pl-[45px] md:pt-0"}`}
                >
                  {" "}
                  ...
                  {/* 연락처 */}{" "}
                  <p className="group mt-[10px] text-[24px] font-bold text-[#292929] sm:text-[28px]">
                    {" "}
                    <a
                      href={`tel:${contact.phoneLink}`}
                      aria-label={`${contact.title} ${contact.phone}로 전화하기`}
                      className="flex w-fit items-center gap-[5px] transition-colors duration-200 hover:text-[#FF6B81]"
                    >
                      {" "}
                      {contact.phone}{" "}
                      <ArrowUpRight className="h-5 w-5 text-[#292929]/60 transition-all duration-300 group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-[#FF6B81] sm:h-6 sm:w-6" />{" "}
                    </a>{" "}
                  </p>{" "}
